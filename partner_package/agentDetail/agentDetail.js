...
Page({
  ...
  agentData: function () {
    ...
    netWork.Htpapi.postPartner("wxProgram/getWxApplyInfoDetail", params).then(res => {
      if (res.data.stateCode == 100) {
        that.setData({
          agentDetail:res.data.data
        })
      }
    })
  },
  ...
  refuse(){
    wx.navigateTo({
      url: '../approveRefuse/approveRefuse?ID='+this.data.id+ '&toBeType=' + '2',
    })
  },
  agree(){
    wx.navigateTo({
      url: '../approveAgree/approveAgree?ID='+this.data.id+ '&toBeType=' + '2',
    })
  }
})